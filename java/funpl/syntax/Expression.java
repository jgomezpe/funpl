...
public class Expression extends Rule{
	...
	protected Token inner_analyze_part2(Lexer lexer, Token command, int start) {
		Source input = command.input();
		...
		Token list = inner_analyze(lexer,current);
	...
	protected Token inner_analyze(Lexer lexer, Token current) {
		...
		if( check_symbol(current, '(')) {
			current = lexer.next();
			command = inner_analyze(lexer, current);
			...
			command = parser.rule(FunConstants.COMMAND).analyze(lexer,current);
			if(command.isError()) return command;
		}
		return inner_analyze_part2(lexer, command, start);
	}
